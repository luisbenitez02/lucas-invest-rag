...
resource "aws_iam_role" "app_runner_instance_role" {
  name = "lucas-app-runner-instance-role"
...
resource "aws_iam_role_policy" "app_runner_instance_bedrock_access" {
  name = "lucas-app-runner-instance-bedrock-policy"
    ...
    image_repository {
      ...
      image_configuration {
        ...
        runtime_environment_variables = {
          ...
          LUCAS_API_ENDPOINT = var.lucas_api_endpoint
          LUCAS_API_KEY      = var.lucas_api_key
        }
      }
...
resource "aws_iam_role" "eventbridge_role" {
  ...
  name  = "lucas-eventbridge-scheduler-role"
...
resource "aws_lambda_function" "scheduler_lambda" {
  count         = var.scheduler_enabled ? 1 : 0
  function_name = "lucas-researcher-scheduler"
...
resource "aws_iam_role" "lambda_scheduler_role" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "lucas-scheduler-lambda-role"
