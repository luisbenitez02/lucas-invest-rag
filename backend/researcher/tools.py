"""
Tools for the Lucas Researcher agent
"""
import os
...
LUCAS_API_ENDPOINT = os.getenv("LUCAS_API_ENDPOINT")
LUCAS_API_KEY = os.getenv("LUCAS_API_KEY")
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            LUCAS_API_ENDPOINT,
            json=document,
            headers={"x-api-key": LUCAS_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the Lucas knowledge base.
    ...
    """
    if not LUCAS_API_ENDPOINT or not LUCAS_API_KEY:
        return {
            "success": False,
            "error": "Lucas API not configured. Running in local mode."
        }
