...
def get_service_url():
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--query", "ServiceSummaryList[?ServiceName=='lucas-researcher'].ServiceArn",
            "--output", "json"
        ], capture_output=True, text=True, check=True)
        
        service_arns = json.loads(result.stdout)
        if not service_arns:
            print("❌ App Runner service 'lucas-researcher' not found.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Test the Lucas Researcher service",
